...
const (
	APIObserver_GetAPIEvent_FullMethodName = "/feeder.APIObserver/GetAPIEvent"
)

// APIObserverClient is the client API for APIObserver service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// APIObserver service for streaming API events
type APIObserverClient interface {
	GetAPIEvent(ctx context.Context, in *ClientInfo, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APIEvent], error)
}

type aPIObserverClient struct {
	cc grpc.ClientConnInterface
}

func NewAPIObserverClient(cc grpc.ClientConnInterface) APIObserverClient {
	return &aPIObserverClient{cc}
}

func (c *aPIObserverClient) GetAPIEvent(ctx context.Context, in *ClientInfo, opts ...grpc.CallOption) (grpc.ServerStreamingClient[APIEvent], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &APIObserver_ServiceDesc.Streams[0], APIObserver_GetAPIEvent_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type APIObserver_GetAPIEventClient = grpc.ServerStreamingClient[APIEvent]

// APIObserverServer is the server API for APIObserver service.
// All implementations should embed UnimplementedAPIObserverServer
// for forward compatibility.
//
// APIObserver service for streaming API events
type APIObserverServer interface {
	GetAPIEvent(*ClientInfo, grpc.ServerStreamingServer[APIEvent]) error
}

// UnimplementedAPIObserverServer should be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedAPIObserverServer struct{}

func (UnimplementedAPIObserverServer) GetAPIEvent(*ClientInfo, grpc.ServerStreamingServer[APIEvent]) error {
	return status.Error(codes.Unimplemented, "method GetAPIEvent not implemented")
}
func (UnimplementedAPIObserverServer) testEmbeddedByValue() {}

// UnsafeAPIObserverServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to APIObserverServer will
// result in compilation errors.
type UnsafeAPIObserverServer interface {
	mustEmbedUnimplementedAPIObserverServer()
}

func RegisterAPIObserverServer(s grpc.ServiceRegistrar, srv APIObserverServer) {
	// If the following call panics, it indicates UnimplementedAPIObserverServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&APIObserver_ServiceDesc, srv)
}

func _APIObserver_GetAPIEvent_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(ClientInfo)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(APIObserverServer).GetAPIEvent(m, &grpc.GenericServerStream[ClientInfo, APIEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type APIObserver_GetAPIEventServer = grpc.ServerStreamingServer[APIEvent]

// APIObserver_ServiceDesc is the grpc.ServiceDesc for APIObserver service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var APIObserver_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "feeder.APIObserver",
	HandlerType: (*APIObserverServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "GetAPIEvent",
			Handler:       _APIObserver_GetAPIEvent_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "kubearmor.proto",
